...
直接将一个数赋给 line-height，而不是一个相对度量(比如 em 或%)，如`line-height=1`表示各元素行高是其各元素自己字体大小的`1倍`，而不是容器元素字体大小的`1倍`。
...
html 中的单位只有一种，那就是像素 px，所以单位是可以省略的，但在 css 中不一样，css 中的单位是必须要写的，因为它没有默认单位。
...
### 第 3 种方式， 给父元素设置 overflow:hidden
